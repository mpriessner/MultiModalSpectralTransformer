...
class MultimodalTransformer(nn.Module):
    ...
    def _create_embeddings_and_masks_1H(self, embedding_src_1H=None, src_padding_mask_1H=None, 
        ...
        # Overwrite MW and MF with zeros and all-ones mask of correct shapes if not None
        if embedding_src_MW is not None:
            embedding_src_MW = torch.zeros_like(embedding_src_MW)
        if src_padding_mask_MW is not None:
            src_padding_mask_MW = torch.zeros_like(src_padding_mask_MW, dtype=torch.bool)
        if embedding_src_MF is not None:
            embedding_src_MF = torch.zeros_like(embedding_src_MF)
        if src_padding_mask_MF is not None:
            src_padding_mask_MF = torch.zeros_like(src_padding_mask_MF, dtype=torch.bool)

        embeddings = []
        masks = []

        if "1H" in self.config.training_mode and embedding_src_1H is not None:
            embeddings.append(embedding_src_1H)
            masks.append(src_padding_mask_1H)
        if "MF" in self.config.training_mode and embedding_src_MF is not None:
            embeddings.append(embedding_src_MF)
            masks.append(src_padding_mask_MF)
        if "MS" in self.config.training_mode and embedding_src_MS is not None:
            embeddings.append(embedding_src_MS)
            masks.append(src_padding_mask_MS)
        if "MW" in self.config.training_mode and embedding_src_MW is not None:
    ...
    # Repeat the same pattern for the other four functions:
    ...
    def _create_embeddings_and_masks_13C(self, embedding_src_13C=None, src_padding_mask_13C=None,
        ...
        if "13C" in self.config.training_mode and embedding_src_13C is not None:
    ...
    def _create_embeddings_and_masks_HSQC(self, embedding_src_HSQC=None, src_padding_mask_HSQC=None, 
        ...
        if "HSQC" in self.config.training_mode and embedding_src_HSQC is not None:
