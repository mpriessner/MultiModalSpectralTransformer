...
def make_predictions(args: Namespace, smiles: List[str] = None) -> List[Optional[List[float]]]:
    ...
    with open(args.preds_path, 'w') as f:
        ...
        for i in range(len(avg_preds)):
            ...
            writer.writerow(row)
            data_for_df.append(row)  # Add the row to the list for DataFrame

    predictions_df = pd.DataFrame(data_for_df, columns=header)

    df_save_path = args.preds_path[:-4]+"_2.csv"
    predictions_df.to_csv(df_save_path, index=False)
