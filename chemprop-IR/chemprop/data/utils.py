...
def get_data_from_smiles(smiles: List[str], skip_invalid_smiles: bool = True, logger: Logger = None, args: Namespace = None) -> MoleculeDataset:
    ...
    if args.spectra_type == "experimental":
        feature_list = [0,0,0,0,0]
        feature_list[args.spectra_type_nr] = 1
        feature_list_arr = np.array(feature_list)
        #feature_data = np.array([1., 0., 0., 0., 0.]) 
        features = [np.array(feature_list_arr)  for smile in smiles] # this defines the use of morgan fingerprints as features
    elif args.spectra_type == "simulated":
        feature_list = [0, 0]
        feature_list[args.spectra_type_nr] = 1
        feature_list_arr = np.array(feature_list)
        #feature_data = np.array([1., 0., 0., 0., 0.]) 
    ...
    data = MoleculeDataset([MoleculeDatapoint(line=[smile], features=feature_data, args=args) for smile, feature_data in zip(smiles, features)])
