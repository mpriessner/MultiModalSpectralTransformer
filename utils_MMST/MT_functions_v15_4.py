...
def _setup_directories_and_config(config):
    if not os.path.exists(config.model_save_dir):
        os.makedirs(config.model_save_dir) # Changed to makedirs for potentially nested paths
...
def _initialize_model(config):
    ...
    if config.load_model:
        ...
        # Ensure the model is loaded correctly for inference or further training
        transformer_multi_gpu_model = TransformerMultiGPU.load_from_checkpoint(
    ...
    if config.use_real_data:
        ...
        if hasattr(transformer_multi_gpu_model.model, 'real_data_linear'):
            print("Unfreezing real_data_linear layer for fine-tuning.")
            for param in transformer_multi_gpu_model.model.real_data_linear.parameters():
                param.requires_grad = True
        else:
            print("Warning: real_data_linear layer not found, though use_real_data is True.")
            
    return transformer_multi_gpu_model


def _setup_trainer(config, logger, checkpoint_callback, profiler):
    trainer = pl.Trainer(
        profiler=profiler,
        gpus=config.gpu_num,
        progress_bar_refresh_rate=10, # consider making this configurable or removing if deprecated
        accelerator='ddp' if config.gpu_num > 1 else None, # ddp only if multiple gpus
        logger=logger,
        callbacks=[checkpoint_callback], # callbacks should be a list
        max_epochs=config.num_epochs,
        # Other potential configurations from original code:
        # fast_dev_run=True,
        # early_stop_callback=early_stopping, # Needs definition if used
...
def _train_model(trainer, model, train_dataloader, test_dataloader, config):
    try:
        trainer.fit(model, train_dataloader, test_dataloader)
    except Exception as e:
        print(f"Error occurred during training: {e}")
        backup_ckpt_path = os.path.join(config.model_save_dir, "last_backup_checkpoint.ckpt")
        trainer.save_checkpoint(backup_ckpt_path)
        print(f"Model saved to {backup_ckpt_path}")


def run_training_MMT(train_dataloader, test_dataloader, config):
    _setup_directories_and_config(config)
    
    checkpoint_callback = _setup_callbacks(config)
    
    # Profiler setup (optional, can be made configurable)
    profiler = SimpleProfiler() 

    # Logger setup
    wandb_logger = WandbLogger(project=config.project, log_model='all')
    config_dict = vars(config) # Log hyperparameters
    wandb_logger.log_hyperparams(config_dict)

    model = _initialize_model(config)
    
    trainer = _setup_trainer(config, wandb_logger, checkpoint_callback, profiler)
    
    _train_model(trainer, model, train_dataloader, test_dataloader, config)
